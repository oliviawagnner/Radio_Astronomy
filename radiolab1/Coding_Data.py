...
# ———————————————————— Leakage Power ————————————————————————————————————————————————————————————————————————————————
voltage_1850 = np.abs(Z3_1850[1])
N_1850 = len(Z3_1850[1])
Hz_1850 = 1850e3 
sampling_frequency = 1.0e6
dt = 1 / sampling_frequency
inc_N = 8 * N_1850 # Oversampling (N_freq >> N)
voltage_pad = np.zeros(inc_N) # Pad with zeros to see leakage power
voltage_pad[:N_1850] = voltage_1850 # Puts original data at front of array, keeps the rest zeros
freq_bins_oversamp = np.fft.fft(voltage_pad)
frequency_oversamp = np.fft.fftfreq(inc_N, d = 1 / sampling_frequency)
power_spec = (np.abs(freq_bins_oversamp)**2)


# ———————————————————— Frequency Resolution ————————————————————————————————————————————————————————————————————————————————
with np.load("L5.6_250_kHz_2048_5mV.npz") as data:
    kHz250_2048samp = data['arr_0']

with np.load("L5.6_250_kHz_16384_5mV.npz") as data:
    kHz250_16384samp = data['arr_0']

with np.load("L5.6_250_kHz_2048_5mV1.npz") as data:
    kHz250_2048samp1 = data['arr_0']

with np.load("L5.6_250_kHz_16384_5mV1.npz") as data:
    kHz250_16384samp1 = data['arr_0']

Xf_2048samp = np.fft.fft(kHz250_2048samp[1])
freq2048 = np.fft.fftfreq(len(kHz250_2048samp[1]), d = 1 / sample_rate)

Xf_2048samp1 = np.fft.fft(kHz250_2048samp1[1])
freq2048_1 = np.fft.fftfreq(len(kHz250_2048samp1[1]), d = 1 / sample_rate)

Xf_2048samp = np.fft.fftshift(Xf_2048samp)
freq2048 = np.fft.fftshift(freq2048)

Xf_2048samp1 = np.fft.fftshift(Xf_2048samp1)
freq2048_1 = np.fft.fftshift(freq2048_1)

voltage_2048 = np.abs(Xf_2048samp)
voltage_2048_1 = np.abs(Xf_2048samp1)

power_2048 = voltage_2048 ** 2
power_2048_1 = voltage_2048_1 ** 2
...
Xf_16384samp = np.fft.fft(kHz250_16384samp[1])
freq16384 = np.fft.fftfreq(len(kHz250_16384samp[1]), d = 1 / sample_rate)

Xf_16384samp1 = np.fft.fft(kHz250_16384samp1[1])
freq16384_1 = np.fft.fftfreq(len(kHz250_16384samp1[1]), d = 1 / sample_rate)

Xf_16384samp = np.fft.fftshift(Xf_16384samp)
freq16384 = np.fft.fftshift(freq16384)

Xf_16384samp1 = np.fft.fftshift(Xf_16384samp1)
freq16384_1 = np.fft.fftshift(freq16384_1)

voltage_16384 = np.abs(Xf_16384samp)
voltage_16384_1 = np.abs(Xf_16384samp1)

power_16384 = voltage_16384 ** 2
power_16384_1 = voltage_16384_1 ** 2
